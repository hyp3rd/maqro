...
/** Pure intermittent-fasting math. No React, no IDB — everything is derived
 *  from `FoodItem.loggedAt` timestamps + the profile's fasting config, so it
 *  stays trivially testable (mirrors `lib/hydration.ts` / `lib/streaks.ts`).
 *
 *  Time model: epoch milliseconds + the browser's local `Date`. A food's
 *  calendar day comes from `new Date(loggedAt)` local getters (same
 *  convention as `hooks/use-today.ts`), so a 00:30 snack counts toward its
 *  own local day. Server-zone helpers (`lib/local-time.ts`) are deliberately
 *  NOT used here — this is client-only, where "local" is the device zone. */
