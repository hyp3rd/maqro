...
export function detectDefaultUnitSystem(): UnitSystem {
  // Read `navigator` off `globalThis` so this stays in `@maqro/core` (no DOM
  // lib): present in browsers, absent server-side and on React Native (where
  // it falls back to metric — native callers pass their own locale instead).
  const nav = (
    globalThis as {
      navigator?: { languages?: readonly string[]; language?: string };
    }
  ).navigator;
  if (!nav) return "metric";
  const languages =
